...
class Controller extends GetxController {
  ...
  RxBool loadingStorePRoducts = true.obs;

  final RxList<StoreProduct> storeProducts = <StoreProduct>[].obs;
  RxList<SpecificStoreProducts> storeProductsDynamic = <SpecificStoreProducts>[].obs;
  RxList<SpecificStoreProducts> SearchedStoreProductsDynamic = <SpecificStoreProducts>[].obs;
  final RxList<StoreProduct> storeProductsMain = <StoreProduct>[].obs;

  final RxString title = 'All Products'.obs;
  final RxString filter = 'Filter'.obs;
  ...
  getAllProductsOfSpecificStore(store_id)async{
    // loadingStorePRoducts = true.obs;
    await ApiController().getSpecificStoreProducts(store_id).then((product) {
      storeProductsDynamic.add(product);
      print('specific store products length : ${storeProductsDynamic[0].data!.length}');
      // loadingStorePRoducts = false.obs;
    });
  }

  resetSearchedPRoductLength(){
    SearchedStoreProductsDynamic.clear();
  }

  getSearchedProductsOfSpecificStore(int store_id,String searchedInput)async{
    // loadingStorePRoducts = true.obs;
    SearchedStoreProductsDynamic.clear();
    await ApiController().getSearchedProductsOfSpecificStore(store_id,searchedInput).then((product) {
      SearchedStoreProductsDynamic.add(product);
      print('Searched products length : ${storeProductsDynamic[0].data!.length}');
      // loadingStorePRoducts = false.obs;
    });
  }
