...
// import 'package:e_plaza/utils/encryption.dart';
...
class ApiProvider {
  ...
  Options get _options {
    return Options(headers: {
      'authorization':
          Preference.isUserAvailable ? Preference.user.authToken : "",
  ...
  Future<List<dynamic>> getList() async {
    ...
    if (enableParamsLogs)
      Logger.m(tag: 'API $endPoint Headers', value: _options.headers);

    try {
      return _dio
          .post(endPoint, data: FormData.fromMap(parameters), options: _options)
          .then((value) {
  ...
  void saveLogToServer({dynamic log = ''}) {
    var data =
        FormData.fromMap({'userId': Preference.user.id, 'log': log.toString()});
